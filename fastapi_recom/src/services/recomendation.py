...
class RecomendationService:
    ...
    async def get_recommendations(self, user_id: UUID, limit: int = 10) -> UserRecommendationResponseDTO:
        similarities = (
            (await self.session.execute(select(UserSimilarity).where(UserSimilarity.user1_id == user_id)))
            .scalars()
            .all()
        )
        ...
        return UserRecommendationResponseDTO(user_id=user_id, recommendations=recommendations[:limit])

    async def get_general_recommendations(self, limit: int = 10) -> GeneralRecommendationResponseDTO:
        subquery = (
            select(Ratings.movie_id).group_by(Ratings.movie_id).having(func.avg(Ratings.rating) >= 7.0).subquery()
        )

        stmt = select(Movies).join(subquery, Movies.id == subquery.c.movie_id).limit(10)
        ...
        for movie in popular_movies:
            similarities = (
                (await self.session.execute(select(MovieSimilarity).where(MovieSimilarity.movie1_id == movie.id)))
                .scalars()
                .all()
            )
