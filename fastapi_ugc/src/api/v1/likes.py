...
async def get_likes_films(
    ...
    service: BaseService = Depends(get_likes_service),
...
async def remove_like(
    ...
    try:
        success = await service.delete(str(like.id), token_payload.user_id)
        if not success:
            raise HTTPException(status_code=404, detail="Лайк не найден")
...
async def update_like(
    ...
    if rating == like.rating:
        raise HTTPException(status_code=400, detail="Новая оценка должна отличаться от текущей")
    try:
        data = {"rating": rating, "user_id": like.user_id, "movie_id": like.movie_id}
