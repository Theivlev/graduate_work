...
import jwt
from src.core.config import mail_queue_settings, project_settings, redis_settings
...
async def send_confirmation_email_task(user):
    token = generate_confirmation_token(
        user_id=str(user.id), redirect_url=mail_queue_settings.redirect_url, secret=project_settings.secret
    )
